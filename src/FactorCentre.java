
public class FactorCentre {
	//public static final float pheromoneFactor = 	(float) 0.5;
	public static final float veporizationFactor = 	(float) 0.978;
	public static final float epocheFactor = 		(float) 0.0;
	public static final float pheromoneExpFactor = 	(float) 2.7;
	public static final float DistanceExpFactor = 	(float) 0.2;
	public static final float PheromoneSpreadFactor = (float) 5.5;
	public static final int   AntPrunningFactor = 	25;
	
	public static final int epocheNumber = 			4000;
	public static final int antNumber = 			1000;	
	
	public static final int graphSize = 			50;
	public static final int maxBestRepets = 		700;
	
	/*
	public static final float veporizationFactor = 	(float) 0.978;
	public static final float epocheFactor = 		(float) 0.0;
	public static final float pheromoneExpFactor = 	(float) 2.7;
	public static final float DistanceExpFactor = 	(float) 0.2;
	public static final float PheromoneSpreadFactor = (float) 5.5;
	public static final int   AntPrunningFactor = 	20;
	
	public static final int epocheNumber = 			3000;
	public static final int antNumber = 			500;	
	
	public static final int graphSize = 			40
	public static final int maxBestRepets = 		600;
	 */
	
	/*
 	public static final float veporizationFactor = 	(float) 0.991;
	public static final float epocheFactor = 		(float) 0.0;
	public static final float pheromoneExpFactor = 	(float) 2.5;
	public static final float DistanceExpFactor = 	(float) 0.2;
	public static final float PheromoneSpreadFactor = (float) 5.5;
	public static final int   AntPrunningFactor = 	15;
	
	public static final int epocheNumber = 			1000;
	public static final int antNumber = 			300;	
	
	public static final int graphSize = 			30; //35
	public static final int maxBestRepets = 		500;
	 */
}
